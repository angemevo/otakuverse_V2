...
class _NewConversationScreenState
  ...
  Future<void> _search(String query) async {
    ...
    try {
      final data = await _supabase
          ...
          .neq('user_id', _uid) // ✅ Exclure moi-même
          .limit(20);
  ...
  Future<void> _openConversation(
    ...
    try {
      debugPrint('🔵 Ouverture conversation avec: $userId');

      final convId = await _service
          .getOrCreateConversation(userId);

      debugPrint('🔵 convId retourné: $convId');

      if (!mounted) return;

      if (convId == null) {
        // ✅ Afficher l'erreur plutôt que silence
      ...
      debugPrint('✅ Navigation vers ChatScreen');
