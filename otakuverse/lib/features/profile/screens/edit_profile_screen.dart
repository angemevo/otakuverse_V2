...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_newAvatar != null) {
        avatarUrl = await _uploadService.uploadImage(_newAvatar!, userId, folder: 'avatars');
      }
      if (_newBanner != null) {
        bannerUrl = await _uploadService.uploadImage(_newBanner!, userId, folder: 'banners');
      }
        ...
        actions: [
          TextButton(
            ...
            child: _isLoading
                ...
                : Text('Sauvegarder',
                    style: GoogleFonts.inter(color: AppColors.crimsonRed, fontWeight: FontWeight.bold, fontSize: 15)),
          ),
        ],
      ),
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  _buildField('Nom affiché', _displayNameController, icon: Icons.person_outline, hint: 'Ton nom visible publiquement'),
                  const SizedBox(height: 16),
                  _buildField('Bio', _bioController, maxLines: 4, maxLength: 500, icon: Icons.info_outline, hint: 'Parle de toi...'),
                  const SizedBox(height: 16),
                  _buildField('Site web', _websiteController, icon: Icons.link, hint: 'https://...'),
                  const SizedBox(height: 16),
                  _buildGenderPicker(),
                ...
                children: [
                  ...
                  Center(child: Column(
                    ...
                    children: [
                      ...
                      Text('Modifier la bannière', style: GoogleFonts.inter(color: AppColors.pureWhite, fontSize: 13)),
                    ],
                  )),
                ],
              ),
          ...
          Positioned(
            ...
            child: GestureDetector(
              ...
              child: Stack(
                children: [
                  Container(
                    decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: AppColors.deepBlack, width: 4)),
                    child: CircleAvatar(
                      radius: 40, backgroundColor: AppColors.darkGray,
                      backgroundImage: _newAvatar != null
                          ? FileImage(_newAvatar!) as ImageProvider
                          : widget.profile.hasAvatar ? NetworkImage(widget.profile.avatarUrl!) : null,
                      child: (_newAvatar == null && !widget.profile.hasAvatar)
                          ? const Icon(Icons.person, color: AppColors.pureWhite, size: 36) : null,
                    ),
                  ),
                  Positioned(
                    bottom: 0, right: 0,
  ...
  Widget _buildField(String label, TextEditingController controller, {int maxLines = 1, int? maxLength, IconData? icon, String? hint}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(
            hintText: hint, hintStyle: GoogleFonts.inter(color: AppColors.mediumGray),
            prefixIcon: icon != null ? Icon(icon, color: AppColors.crimsonRed, size: 20) : null,
      ...
      children: [
        ...
        Container(
          ...
          child: DropdownButton<String>(
            value: _selectedGender, isExpanded: true, dropdownColor: AppColors.darkGray,
  ...
  Widget _buildFavoritesSection() {
    ...
    return Column(
      ...
      children: [
        Row(children: [
          Text('Préférences', style: GoogleFonts.poppins(color: AppColors.pureWhite, fontWeight: FontWeight.w600, fontSize: 16)),
          const Spacer(),
          Text('Modifiable bientôt', style: GoogleFonts.inter(color: AppColors.mediumGray, fontSize: 11)),
        ]),
        ...
        if (widget.profile.favoriteGenres.isNotEmpty) ...[
          ...
          Wrap(spacing: 8, runSpacing: 8, children: widget.profile.favoriteGenres.map((g) => _chip(g)).toList()),
          const SizedBox(height: 16),
        ],
        if (widget.profile.favoriteAnime.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 8, children: widget.profile.favoriteAnime.map((a) => _chip(a)).toList()),
        ],
      ],
    );
  }
  ...
  Widget _chip(String label) {
    return Container(
      ...
      child: Text(label, style: GoogleFonts.inter(color: AppColors.lightCrimson, fontSize: 12, fontWeight: FontWeight.w500)),
    );
  }
}